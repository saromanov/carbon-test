...
def home():
    ...
    '''
    last_records = Statistics.query.order_by(
        desc(Statistics.id)).limit(100).all()
    min_data = aggregate(func.min)
    max_data = aggregate(func.max)
    avg_data = aggregate(func.avg)

    last_100_min = db.session.query(
        Statistics, func.min(
            Statistics.cpu_load)).order_by(
        desc(
            Statistics.id)).limit(5).all()
    last_100_max = db.session.query(
        Statistics, func.max(
            Statistics.cpu_load)).order_by(
        desc(
            Statistics.id)).limit(5).all()
    last_100_avg = db.session.query(
        Statistics, func.avg(
            Statistics.cpu_load)).order_by(
        desc(
            Statistics.id)).limit(5).all()
...
def insert_data():
    ...
    if value < 0 or value > 100:
        raise Exception(
            'load parameter have invalid data. Should be in percent ')
